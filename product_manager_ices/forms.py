...
class AddOrderItem(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["flavour"].choices = [
            (flav.id, flav.flavour) for flav in Flavour.objects.all()
        ]
        # this is to do not restart server to show the added product automatically (dynamic add)
        self.fields["ice"].choices = [
            (typeice.id, typeice.type) for typeice in Ices.objects.all()
        ]
